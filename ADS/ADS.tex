...
\end{lstlisting}
If we wanted to create a jagged array of integers called \inline!a!, with 4 rows, and the $i^{th}$ row has $i$ columns, and \inline!array[i][j]! has value \inline!i+j!, we could write the following
\vspace{-15pt}
\begin{lstlisting}
int[][] a = new int[4][];
for(i = 0; i < 4; i++)
