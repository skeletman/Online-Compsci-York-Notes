...
\end{defi}
\begin{impl}[Stack]
    A stack $S$ will have 2 attributes: a positive integer $S.top$, and an array $S$
    \begin{algorithm}
...
\end{impl}
\begin{sidenote}
    Given that arrays are finite, an array overflow may occur when performing Push. We have ignored overflows in our pseudocode implementation.
\end{sidenote}
\pagebreak
\begin{defi}[Queue]
    A \term{stack} is a collection equipped with two operations.
...
\end{defi}
\begin{impl}[Stack]
    A stack $Q$ will have 3 attributes: An array $Q$, and positive integers $Q.head$ and $Q.tail$.
    \begin{algorithm}
        \caption{Enqueue($Q, x$)}
        $Q[Q.tail] \gets x$\;
        \uIf{}
...
\end{impl}
\begin{sidenote}
    If we attempt to more than $Q.length$ elements, we would cause some elements to be overwritten. We have ignored overflows and underflows in our pseudocode implementation.
\end{sidenote}
\pagebreak
\chapter{Algorithms}
