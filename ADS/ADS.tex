...
A simple sorting algorithm is \term{insertion sort}. The idea is that, if the first $i$ elements of our list are sorted, we insert the $(i+1)^{th}$ element in the correct position to obtain a sorting of the first $i+1$ elements. Repeating this process we can sort the whole list.  
\begin{algorithm}
