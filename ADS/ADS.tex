...
\end{nproblem}
\begin{sidenote}
    Sometimes we have more than just comparison data, which can be useful to sort a list more quickly. Here we only consider algorithms that use only the comparison data. 
\end{sidenote}
\begin{note}
    Currently, for the purposes of computation, we will assume that the sequence is stored in an array. Later, we will consider structures which impose more restrictions on how the data is stored in order to improve the running time of operations such as searches and sorts.
\end{note}
\subsection{Insertion Sort}
A simple sorting algorithm is \term{insertion sort}. The idea is that, if the first $i$ elements of our list are sorted, we insert the $(i+1)^{th}$ element in the correct position to obtain a sorting of the first $i+1$ elements. Repeating this process we can sort the whole sequence.  
\begin{algorithm}
    \caption{Insertion-Sort($A$)}
    \For{$i = 2$ \KwTo $A.length$}
    {
        $key = A[i]$\;
        $j = i - 1$\;
...
Let $n$ be the length of the input sequence, $A.length$. We define $t_i$ to be the number of iterations of the inner while loop for a fixed $i$ in the outer for loop.
\mypar
\begin{algorithm}[H]
    \caption{Insertion-Sort($A$)}
    \For(\tcp*[f]{cost $c_1$, times $n$}){$i = 2$ \KwTo $A.length$}
    {
        $key = A[i]$\tcp*{cost $c_2$, times $n-1$}
        $j = i - 1$\tcp*{cost $c_3$, times $n-1$}
        \While(\tcp*[f]{cost $c_4$, times $\sum_{i = 2}^{n}t_i$}){$j > 0$ and $A[j] > key$}
        {
            $A[j+1] = A[j]$\tcp*{cost $c_5$, times $\sum_{i = 2}^{n}t_i-1$}
            $j = j - 1$\tcp*{cost $c_6$, times $\sum_{i = 2}^{n}t_i-1$}
        }
        $A[j+1] = key$\tcp*{cost $c_7$, times $n-1$} 
    }
\end{algorithm}
\mypar
We see that each iteration of the while loop decreases $j$ by $1$, and $j$ is initially set at $i-1$. The loop terminates when $j$ reaches $0$, so $1\le t_i\le i-1$.
\\ We then see that the running time $T$ on an input of size $n$ satisfies, for some constants $A,B,C,A',B',C',D'$,
\[A + Bn+ C\sum_{i = 2}^{n}1 \le T \le  A' + B'n + C'\sum_{i = 2}^{n}i + D'\sum_{i = 2}^{n}(i-1)  \]
So 
\[A + Bn+ C(n-1) \le T \le  A' + B'n + C'\left(\frac{n(n-1)}{2}-1\right) + D'\frac{(n-1)(n-2)}{2}  \]
Looking at the lower bound, we get $T = \Omega(n)$, and at the upper bound $T = O(n^2)$.
\printindex
\end{document}