...
Once a Java program is compiled, running the program will execute the \inlineJava{main}method in the class of the .java file that was compiled. In listing \ref{lst:HelloWorld}, the \inlineJava{main} method is executed and hence the words "Hello World" are printed to the console when the program is executed.
...
An \index{ArrayList}\inlineJava{ArrayList<T>} object stores an array of type \inlineJava"T", and if an index needs to be accessed beyond the array's size, dynamically resizes the array. As a result, we obtain a data structure that can store an arbitrary number of elements.
