...
When analysing the runtime or memory usage of programs, the constants are often unimportant. Hence we often classify these using \term{asymptotic bounds}, which tell us about the behaviour of our programs with large inputs. 
\begin{intuition}
...
\end{egs}
\begin{eg}
    We calculated the runtime of algorithm \ref{alg:max} to be $a+e+(b+c+d)n$ for input an array of length $n$, for some constants $a,b,c,d$ and $e$.
    We see that this grows linearly, independent of input, so the runtime is $\Theta(n)$.
\end{eg}
\pagebreak
\begin{eg} A problem, algorithm, and runtime analysis.
    \begin{nproblem}
    ...
    \begin{itemize}
        \item The runtime of LinearSearch is $O(n)$.
        \item The runtime of LinearSearch is $\Omega(1)$.
        \item The best-case runtime of LinearSearch is $\Theta(1)$.
        \item The worst-case runtime of LinearSearch is $\Theta(n)$.
    \end{itemize}
\end{eg}
\pagebreak
...
\begin{defi}
    [Singly Linked List]
    In a \term{singly linked list} each element is stored alongside a pointer to the next element.
\end{defi}
\begin{figure}[H]
    \caption{A singly linked list $L$ representing the set $\{3,101,5,2,19\}$}
    \centering
    \begin{tikzpicture}[cell/.style={rectangle split, rectangle split horizontal , rectangle split parts=2, draw}]
        \node[cell] (cell1) 
        {
            \nodepart{one}{19}
        };
        ...
        \draw[->] (cell1.two east) -- (cell2.one west);
        \draw[->] (cell2.two east) -- (cell3.one west);
        \draw[->] (cell3.two east) -- (cell4.one west);
        \draw[->] (cell4.two east) -- (cell5.one west);
        \draw[->] (headtext.north) -- (cell1.south);
        \draw[->] (tailtext.north) -- (cell5.south);
        \draw[->] (keytext.south) -- (cell1.one north);
        \draw[->] (nexttext.south west) -- (cell1.two north);
    \end{tikzpicture}
\end{figure}
\begin{defi}
    [Doubly Linked List]
    In a \term{doubly linked list} each element is stored alongside pointers to the next and previous elements.
\end{defi}
\begin{figure}[H]
    \caption{A doubly linked list $L$ representing the set $\{0,541,5,12\}$}
    \centering
    \begin{tikzpicture}[cell/.style={rectangle split, rectangle split horizontal , rectangle split parts=3, draw}]
        \node[cell] (cell1) 
        {
            \nodepart{one}{$\bullet$}
            \nodepart{two}{12}
        ...
        \draw[->] ($(cell1.south east)!.7!(cell1.north east)$) -- ($(cell2.south west)!.7!(cell2.north west)$);
        \draw[->] ($(cell2.south east)!.7!(cell2.north east)$) -- ($(cell3.south west)!.7!(cell3.north west)$);
        \draw[->] ($(cell3.south east)!.7!(cell3.north east)$) -- ($(cell4.south west)!.7!(cell4.north west)$);
        \draw[->] ($(cell2.south west)!.3!(cell2.north west)$) -- ($(cell1.south east)!.3!(cell1.north east)$);
        \draw[->] ($(cell3.south west)!.3!(cell3.north west)$) -- ($(cell2.south east)!.3!(cell2.north east)$);
        \draw[->] ($(cell4.south west)!.3!(cell4.north west)$) -- ($(cell3.south east)!.3!(cell3.north east)$);

        \draw[->] (headtext.north) -- (cell1.south);
        \draw[->] (tailtext.north) -- (cell4.south);
        \draw[->] (keytext.south) -- (cell1.two north);
        \draw[->] (prevtext.south east) -- (cell1.one north);
        \draw[->] (nexttext.south west) -- (cell1.three north);
    \end{tikzpicture}
\end{figure}
\begin{defi}
